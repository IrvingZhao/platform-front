<template>
    <el-container class="pj-main">
        <el-aside class="aside" :class="{'layout-hide-text': isCollapse}" :width="isCollapse?'64px':'135px'">
            ...
            <el-menu class="aside-menu" v-mock-scroll="" :router="true" :collapse="isCollapse"
                     :collapse-transition="false" @open="menuOpen" @close="menuClose">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            menuSelect(menuIndex, menuIndexTree) {
                let menuItem = {children: this.menuList};
                for (let i = 0; i < menuIndexTree.length; i++) {
                    menuItem = menuItem.children[menuIndexTree[i]];
                }
                if (menuItem.path) {
